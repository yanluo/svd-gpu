...
__global__ void thread_eigvecr_v3(float *d_vec, float *d_a, float *d_b, U32 n, U32 threads, float *d_val, float *d_l, float *d_u, float *d_dl, float *d_du)
{
...
__global__ void thread_eigvecl_v3(float *d_vec, float *d_a, float *d_b, U32 n, U32 threads, float *d_val, float *d_l, float *d_u, float *d_dl, float *d_du)
{
   U32 k = blockIdx.x * blockDim.x + threadIdx.x;
   if (k > threads) return;
   float dl, du, sl, su, val;
...
void par_eigenMat_v3(float *p_lvec, float *p_rvec, float *d_a, float *d_b, U32 n, U32 n_lo, U32 n_up, float *p_val)
{
   ...
   for (U32 i=0; i<iters; i++){
      thread_eigvecr_v3<<<n_blocks,MAX_VEC_THREAD>>>(d_vec, d_a, d_b, n, k, d_val, d_l, d_u, d_dl, d_du);
      cudaErrors(cudaGetLastError());
      cudaErrors(cudaDeviceSynchronize());
   }
   cudaErrors(cudaMemcpy(p_rvec, d_vec, size, cudaMemcpyDeviceToHost));
#ifdef TIME
...
#endif
   for (U32 i=0; i<iters; i++){
      thread_eigvecl_v3<<<n_blocks,MAX_VEC_THREAD>>>(d_vec, d_a, d_b, n, k, d_val, d_l, d_u, d_dl, d_du);
      cudaErrors(cudaGetLastError());
      cudaErrors(cudaDeviceSynchronize());
   }
   cudaErrors(cudaMemcpy(p_lvec, d_vec, size, cudaMemcpyDeviceToHost));
#ifdef TIME
   cudaEventRecord(stop, 0);
   cudaErrors(cudaEventSynchronize(stop));
   cudaErrors(cudaEventElapsedTime(&time, start, stop));
